...
import com.joy.log.config.LogConfig;
import com.joy.log.utils.ObjectUtils;
import com.joy.log.utils.Utils;
...
import static com.joy.log.LogLevel.TYPE_DEBUG;
import static com.joy.log.LogLevel.TYPE_ERROR;
import static com.joy.log.LogLevel.TYPE_INFO;
import static com.joy.log.LogLevel.TYPE_VERBOSE;
import static com.joy.log.LogLevel.TYPE_WARN;
...
public class PrinterService implements IPrinter {
    ...
    public static void print(@LogLevel.LogLevelType int level, String tag, boolean isPart, String
            msg) {
        ...
        if (msg.length() > LogConfig.LINE_MAX) {
            ...
            print(level, tag, createMessage(msg));
        }
    ...
    private static String getLineIndicator() {
        StackTraceElement element = ((new Exception()).getStackTrace())[LogConfig.STACK_TRACE];
        StringBuffer sb = new StringBuffer("(").append(element.getFileName()).append(":").append
                (element.getLineNumber()).append(").").append(element.getMethodName()).append(":");
        return sb.toString();
    }
    ...
    private static String createMessage(String msg) {
        if (LogConfig.LINE_INDICATOR_ENABLE) {
            return "[Thread： " + getTreadName() + "] " + "[" + getLineIndicator() + "] " + "[Msg：" +
                    " " + msg + "] ";
        } else {
            return "[Thread： " + getTreadName() + "] " + "[Msg： " + msg + "] ";
